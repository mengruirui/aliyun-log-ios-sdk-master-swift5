...
@objc open class LogGroup:NSObject{
    ...
    @objc public init(topic:String,source:String){
        mTopic = topic
        mSource = source
    }

    @objc open func PutTopic(_ topic:String){
        mTopic = topic
    }
    @objc open func PutSource(_ source:String){
        mSource = source
    }
    @objc open func PutLog(_ log:Log){
