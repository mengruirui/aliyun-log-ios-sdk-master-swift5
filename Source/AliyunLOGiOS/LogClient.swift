...
import Foundation
@objc public class LOGClient: NSObject {
    ...
    @objc public var mIsLogEnable: Bool = false
    @objc public var mConfig: SLSConfig
    
    fileprivate var cacheManager: CacheCheckManager?
    
    @objc public init(endPoint:String,accessKeyID:String,accessKeySecret :String,projectName:String, token: String? = nil, config: SLSConfig = SLSConfig()){
        if( endPoint.range(of: "http://") != nil ||
            endPoint.range(of: "Http://") != nil ||
            endPoint.range(of: "HTTP://") != nil){
            mEndPoint = endPoint.substring(from: endPoint.index(endPoint.startIndex, offsetBy: 7))
        } else if( endPoint.range(of: "https://") != nil ||
            endPoint.range(of: "Https://") != nil ||
            endPoint.range(of: "HTTPS://") != nil){
            mEndPoint = endPoint.substring(from: endPoint.index(endPoint.startIndex, offsetBy: 8))
        } else{
            mEndPoint = endPoint
        }
    ...
    @objc public convenience init(endPoint:String, accessKeyID:String, accessKeySecret:String, token:String, projectName:String, config: SLSConfig){
    ...
    @objc open func PostLog(_ logGroup:LogGroup,logStoreName:String, call: @escaping (URLResponse?, NSError?) -> ()){
